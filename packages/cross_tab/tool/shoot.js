// Screenshots the running example via the Chrome DevTools Protocol.
//
// Chrome's --screenshot flag is driven by --virtual-time-budget, which waits
// for the page to go idle. cross_tab keeps a heartbeat timer running by
// design, so that page is never idle and the flag hangs forever. CDP lets us
// wait a real number of seconds instead.
//
...
const [WIDTH, HEIGHT] = (process.env.SIZE ?? '1400x520').split('x').map(Number);

const shots = [
  { name: 'leader', query: 'auto=1', settle: 3500 },
  { name: 'election', query: 'auto=1&drop=1', settle: 5200 },
];
